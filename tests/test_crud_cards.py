...
def get_or_create_card(session, question, reponse, probabilite, id_theme):
    is_created = False
    card = session.query(Card).filter_by(question=question, reponse=reponse, probabilite=probabilite, id_theme=id_theme).first()
    if card is None:
...
def ensure_card_exists():
    with config.get_session() as session:
        card = session.query(Card).filter_by(id=1, question="What is Python?", reponse="A programming language", probabilite=0.5, id_theme=2).first()
    if card is None:
