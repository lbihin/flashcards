...
@given(
    parsers.parse('a card exists with ID {card_id} and probability {probability}')
)
def ensure_card_exists(card_id, probability):
    new_card = create_card(question="question", reponse="reponse", probabilite=probability, id_theme=2)
    assert new_card.id == int(card_id)

@when(parsers.parse('the card with ID {card_id} with {is_correct} answer is updated'))
def update_card_probability_with_id(card_id, is_correct: Literal['correct', 'incorrect']):
    correct = is_correct == "correct"
    update_card_probability(card_id=int(card_id), is_correct=correct)

@then(parsers.parse('the card with ID {card_id} should have the probability {probability}'))
...
@then(parsers.parse('an error should be logged indicating the card with ID {card_id} was not found'))
...
@then(parsers.parse('the stat should have {positive_answer_cnt} correct answers and {negative_answer_cnt} incorrect answers'))
